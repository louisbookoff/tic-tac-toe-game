...
var gameEngine = __webpack_require__(13);
...
var onCreateGame = function onCreateGame() {
  event.preventDefault();
  api.createGames().then(ui.createGameSuccess).catch(ui.createGameFailure);
  gameEngine.createBoard = new Array(9).fill('');
  gameEngine.players.currentPlayer = gameEngine.playerOne;
};

var onUpdateGames = function onUpdateGames(event) {
  event.preventDefault();
  // get board position
  ...
  if (gameEngine.createBoard[attribute] !== '') {
    ...
    // checking winner
    var won = gameEngine.checkIfWinner(gameEngine.createBoard);
...
var onGetGames = function onGetGames() {
  event.preventDefault();
  api.getAllGames().then(ui.getGamesSuccess).catch(ui.getGamesFailure);
};
...
var inGameMessages = function inGameMessages(createBoard) {
  ...
  createBoard[6] === createBoard[4] && createBoard[4] === createBoard[2] && createBoard[6] !== '') {
    playerTurn();
    var messageText = 'the winner is ' + players.currentPlayer;
    $('#message').text(messageText);
    $('#message').css('background-color', 'white');
  } else if (createBoard[0] === '' || createBoard[1] === '' || createBoard[2] === '' || createBoard[3] === '' || createBoard[4] === '' || createBoard[5] === '' || createBoard[6] === '' || createBoard[7] === '' || createBoard[8] === '') {
...
var checkIfWinner = function checkIfWinner(createBoard) {
  // Horizontal top row
  // 0,1,2
  if (createBoard[0] === createBoard[1] && createBoard[1] === createBoard[2] && createBoard[2] !== '' ||
